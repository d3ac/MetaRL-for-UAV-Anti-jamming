...
class Agent:
    ...
    def train(self):
        ...
        if self.buffer.is_available():
            ...
            sample_index = np.random.randint(0, len(memo)-1)

            obs_list = []
            action_list = []
            reward_list = []
            obs_next_list = []
            for i in range(sample_index, len(memo)):
            ...
            obs_list = (np.array(obs_list)).reshape(-1, 1, self.eval_net.input)
            obs_list = torch.FloatTensor(obs_list)
            obs_next_list = (np.array(obs_next_list)).reshape(-1, 1, self.eval_net.input)
            ...
            q_eval4next = np.squeeze(q_eval4next.detach().numpy())

            q_next = q_next.squeeze()

            q_target = q_eval.clone()
            batch_index = np.arange(len(range(sample_index, len(memo))), dtype=np.int32)
            max_act4next = np.argmax(q_eval4next, axis=-1)
            max_postq = q_next[batch_index, max_act4next]

            for t in range(len(batch_index)):
