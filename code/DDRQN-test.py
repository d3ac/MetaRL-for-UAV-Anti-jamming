    ...
    for N_TASK in TR:
        ...
        for i_episode in Trange:
            ...
            for step in range(n_steps):
                ...
                for i in range(n_agent):
                    action_all[i], hidden_all[i] = env.agents[i].get_action(obs[i], hidden_all[i], get_decay(i_episode))
                ...
                if env.agents[0].buffer.is_available() and steps % train_frequency == 0:    # 随便检查一个的经验池即可
                    for i in range(n_agent):
                        env.agents[i].train((hidden_all[i][0].detach().clone(), hidden_all[i][1].detach().clone()))
