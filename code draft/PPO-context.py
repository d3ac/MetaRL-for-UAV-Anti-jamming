...
from ppoalg.uav_config import uav_config
from ppoalg.env_utils import ParallelEnv, LocalEnv
from ppoalg.uav_model import uavModel_context
from ppoalg.agent import Agent
from ppoalg.storage import RolloutStorage
from ppoalg.multiPPO import PPO
...
def main():
    ...
    # model
    model = uavModel_context(obs_space, act_space, n_clusters, args.device)
    ppo = PPO(
    ...
    for update in Trange:
        ...
        hidden_all = []
        for i in range(env.env.n_ch):
            hidden = (torch.zeros(2, 1, 128).float(), torch.zeros(2, 1, 128).float())
            hidden_all.append(hidden)

        # pre sample
        done_template = np.array([False] * env.n_clusters)
        for t in range(pre_step):
            value, action, log_prob, _, hidden_all = agent.sample(obs, hidden_all)
            next_obs, reward, next_done, info = env.step(action)
            next_done = done_template
        ...
        for t in range(max_step):
            ...
            if t % update_period == 0 and rollout.get_len() > batch_size:
                value = agent.value(obs, hidden_all)
                rollout.compute_returns(value, done_template)
                agent.learn(rollout, hidden, batch_size)
