...
class Agent(parl.Agent):
    ...
    def sample(self, obs, hidden_all):
        obs = torch.tensor(obs)
        value, action, action_log_probs, action_entropy, new_hidden = self.alg.sample(obs, hidden_all)
        ...
        return value, action, action_log_probs, action_entropy, new_hidden
    
    def value(self, obs, hidden_all):
        obs = torch.tensor(obs)
        value, h = self.alg.value(obs, hidden_all)
        value = np.array([value[i].cpu().detach().numpy() for i in range(self.alg.model.n_clusters)])
        return value
    
    def learn(self, rollout, hidden_all, batch_size=32):
        # lr
        if self.config['lr_decay']:
        ...
        value_loss, action_loss, entropy_loss = self.alg.learn(batch_obs, batch_action, batch_value, batch_return, batch_log_prob, batch_adv, hidden_all, lr)